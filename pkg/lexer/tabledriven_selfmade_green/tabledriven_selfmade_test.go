...
func TestLexer_Peek_Read(t *testing.T) {
	for _, tt := range [...]struct {
		...
		if tt.expectPeekErr && err == nil {
			t.Errorf("want error, got nil [tt: %s]", tt.name)
		}
		if !tt.expectPeekErr && err != nil {
			t.Error(err)
		}
		if !tt.expectPeekErr && out != tt.expectPeekKeyword {
			t.Errorf("want: %s, got: %s [tt: %s]", tt.expectPeekKeyword.String(), out.String(), tt.name)
		}
		...
		if actualToken.Keyword != tt.expectReadKeyword {
			t.Errorf("want: %s, got: %s [tt: %s]", tt.expectReadKeyword.String(), actualToken.Keyword, tt.name)
		}
	}
}
